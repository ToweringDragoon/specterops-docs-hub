...
The following are some general guidelines to follow while writing and submitting your pull requests. Following these should make merging the pull request faster and easier.

>[!TIP]
>Check out [Contributing to Docs](https://github.com/SpecterOps/bloodhound-docs/blob/main/./Contributing-To-Docs.md) for specifics!

### Before opening an issue or pull request

If you're planning to submit a pull request, please ensure a GitHub issue has been opened first. The reasons for this are:
...
All pull request submissions will need to come from a separate branch from `main`. We recommend creating a personal fork of this repo and creating the branch from there. This will allow you more control over your local environment.
...
Commit messages should be in conventional commit style as described in the [Conventional Commits Guide RFC](https://github.com/SpecterOps/BloodHound/blob/main/rfc/bh-rfc-2.md). Please take some time to review the guidelines in order to format your commits accordingly so that the work passes the associated pull request check. 

Adding well formatted and detailed commit messages will help with the review process as it will be easier to see the reasoning and details included in a particular change. Another habit that will help the pull request reviewer is authoring smaller, frequent commits as opposed to a giant single commit containing all of your changes.

### Pull request etiquette

If you are participating in the review of a pull request then you should add yourself to the Reviewers list. It sends appropriate notifications to all reviewers when the pull request author requests an additional review after making changes, so be sure to use that button within GitHub when you are ready for an additional look.

It should be noted that the presence of a reviewer on a pull request should not dissuade others from also reviewing the pull request. The more eyes the better.

If you will no longer be reviewing a pull request for some reason, then please try to remember to remove yourself as a Reviewer.
