...
OpenHound uses DLT configuration management for CLI asset uploads. Add the BloodHound Enterprise tenant URL to your local `~/.dlt/config.toml` file, and the browser JWT to your local `~/.dlt/secrets.toml` file.

The asset upload commands use the `destination.bloodhound` section with a URL and a JWT bearer token.
...
<Note>
  ...
  Asset upload commands run from the OpenHound CLI and read the generic `~/.dlt/config.toml` and `~/.dlt/secrets.toml` files shown below.
</Note>
...
<Note>
  If an existing `~/.dlt/secrets.toml` file still has `url` under `[destination.bloodhound]` from a previous OpenHound version, it continues to work, but following the new pattern is preferred. Use `config.toml` for `url` in new configurations.
</Note>
...
| Upload fails with an authentication error | The BloodHound Enterprise URL or JWT is invalid or expired. | Verify both values; update `~/.dlt/config.toml` with a valid URL and `~/.dlt/secrets.toml` with a valid browser JWT as needed, then rerun the command. |
