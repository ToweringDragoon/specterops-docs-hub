...
The Okta collector is a data collector for [Okta Platform](https://www.okta.com/products/workforce-identity/) (also known as Okta Workforce Identity Cloud) environments that helps security professionals visualize and analyze their Okta configurations in BloodHound.

It collects data about users, groups, applications, roles, and other entities within an Okta organization and represents them as [nodes](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/extensions/okta/schema#nodes) and [edges](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/extensions/okta/schema#edges) in BloodHound's graph database.

<Note>The other main product in Okta's portfolio is [Auth0](https://auth0.com/) (previously known as Customer Identity Cloud). The Okta collector **does not** currently support Auth0.</Note>
...
Okta provides a [free trial](https://developer.okta.com/signup/) plan that you can use to test the majority of Okta collector features.
...
Please join us in the `#okta` channel of the [BloodHound Community Slack](https://slack.specterops.io/) workspace if you want to chat about attack paths in Okta or the usage of the Okta collector. You are also welcome to open an issue or pull request on [GitHub](https://github.com/SpecterOps/openhound-okta/).
