...
<Steps>
  ...
  <Step title="Create the collector-specific secrets file">
    ...
    The example Compose file uses [collector-specific](https://github.com/SpecterOps/bloodhound-docs/blob/main//openhound/configuration#collector-specific-secrets) secrets files on the host and maps the matching file into the container as `/app/.dlt/secrets.toml`.
    ...
    <Note>
      ...
      Do not use the JWT-based `destination.bloodhound` settings from [Upload Extension Assets](https://github.com/SpecterOps/bloodhound-docs/blob/main//openhound/upload-extension-assets) for scheduled collection.
    </Note>
  </Step>
