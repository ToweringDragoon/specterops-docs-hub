...
There are several options for this step. You can obtain a session as SYSTEM on the host, which allows you to interact with AD as the computer account, by abusing control over the computer AD object (see [GenericAll edge](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/edges/generic-all) documentation).

Step 5: Enroll certificate as victim.

Use Certify (2.0) as the victim computer to request enrollment in the affected template, specifying the affected [EnterpriseCA](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/enterprise-ca):
