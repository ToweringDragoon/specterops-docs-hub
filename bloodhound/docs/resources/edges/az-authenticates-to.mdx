...
Any principal that can obtain a token from the FIC's trusted issuer matching its subject claim can authenticate as the App Registration, which in turn runs as its associated Service Principal via the [AZRunsAs](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/edges/az-runs-as) relationship.
...
From there, follow the [AZRunsAs](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/edges/az-runs-as) edge to understand what Service Principal context, and associated permissions, the attacker gains.
