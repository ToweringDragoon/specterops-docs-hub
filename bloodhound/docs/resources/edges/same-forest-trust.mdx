...
See the [SpoofSIDHistory](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/edges/spoof-sid-history) edge for more information.
...
See the [CoerceToTGT](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/edges/coerce-to-tgt) edge for more information.
...
AD sites are stored in the forest-wide Configuration NC partition, writable by any DC within the forest. An attacker with SYSTEM access to a DC can link a malicious [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo) to the site of any DC in the forest.
...
**Step 2: Create a [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo)**
Use the GroupPolicy module of RSAT to create the new [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo):

```powershell
New-GPO -Name "MyGPO"
```

**Step 3: Add the compromising setting to the [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo)**
...
**Step 5: Set the [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo) permissions**

This step is important to avoid applying the [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo) to all computers connected to the site. Use the GroupPolicy module of RSAT to modify the permissions such that Authenticated Users can read the object but only the targeted computer applies the GPO settings:
...
**Step 6: Link the [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo) to the site**
Use the GroupPolicy module of RSAT to link the [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo) to the site:
...
Note that you must specify the server to be the DC where you are running the command, as the command defaults to execute the change on a root domain DC where the compromised DC does not have the permissions to link the [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo).

Wait until replication has happened and the [GPO](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/nodes/gpo) has applied on the target DC, and log in with Administrators access on the compromised DC. Replication within the same site happens within 15 seconds but runs on 3 hour schedule by default across sites. GPOs are applied on a 90-120 min interval by default.
