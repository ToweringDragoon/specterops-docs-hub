...
<Update label="BloodHound" description="Enhancement" tags={["OpenGraph"]}>
  ...
  New [property-based edge matching](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/developer/edges#match-by-property) (`match_by: "property"`) enables hybrid edge creation using cross-system attributes, such as email, username, or hostname.
...
<Update label="BloodHound" description="Breaking Change" tags={["OpenGraph"]}>
  ...
  To keep node identity unambiguous for property-based endpoint matching workflows, `objectid` is now [reserved](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/developer/nodes#objectid) and may not be defined in a node's `properties` object.
