...
Enabling this option has BloodHound create a new user on the first login with SSO (Just-In-Time). BloodHound assigns the role in the role claim only when **Allow IDP to modify roles** is enabled and the assertion includes exactly one valid BloodHound role. Otherwise, BloodHound assigns the default role.
...
BloodHound will provide the URLs related to this new {auth_mode} provider integration. Please take a moment to verify that the **{endpoint} URL** matches the **Single sign on URL** specified in the {auth_mode} application integration page during setup of the {auth_mode} integration.
