...
| `markdown.content` | Markdown rendered in the section body. Required when `markdown` is present. Validated on upload. See [Markdown validation](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/developer/graph-definition#markdown-validation). |
...
| Field(s) | Where |
| --- | --- |
| `info.<section>.markdown.content` | `node_kinds` and `relationship_kinds` arrays (see [Custom Entity Panel content](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/developer/graph-definition#custom-entity-panel-content)) |
...
<ResponseField name="info" type="object">
  Optional [custom Entity Panel content](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/developer/graph-definition#custom-entity-panel-content) for nodes of this kind.
</ResponseField>
...
<ResponseField name="info" type="object">
  Optional [custom Entity Panel content](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/developer/graph-definition#custom-entity-panel-content) for relationships of this kind.
</ResponseField>
...
Defines findings based on relationships. Each finding represents a single instance of an <Tooltip tip="A chain of abusable privileges and user behaviors that creates direct or indirect connections between principals." cta="Learn more" href="/analyze-data/findings/attack-paths">Attack Path</Tooltip>. BloodHound Enterprise identifies instances of a traversable edge targeting a node of higher privilege (violating a Privilege Zone boundary), analyzes the risk, and provides remediation guidance.
...
<ResponseField name="remediation" type="object">
  Remediation guidance to resolve the finding, including both short and long forms. The `short_description`, `long_description`, `short_remediation`, and `long_remediation` fields accept Markdown and are validated on upload. See [Markdown validation](https://github.com/SpecterOps/bloodhound-docs/blob/main//opengraph/developer/graph-definition#markdown-validation).
</ResponseField>
