...
[HasSession](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/edges/has-session) edges are generated to indicate patterns of behavior rather than session active at any exact moment. For this reason, HasSession edges are only reconciled based on their retention/time-to-live expiring, rather than reconciling upon follow-on collections no longer seeing the active session.
...
* Session Data, i.e. [HasSession](https://github.com/SpecterOps/bloodhound-docs/blob/main//resources/edges/has-session) edges: 3 days
...
To implement this, BHE stores a timestamp on every data point, updated whenever a new collection includes the same data point. The timestamp on nodes can be seen as the "Last Seen by BloodHound" property in every node's entity panel on the **Explore** page.

<Frame>
  <img src="/images/data_collectors/data-retention-last-seen.png" alt="Last Seen by BloodHound property in a node's entity panel" style={{ width: "50%" }}/>
</Frame>

<Note>
In cases where retention maintains visibility into an already resolved finding, the "Accept" feature may be used to hide nodes/principals in the "Attack Paths" page, see [Accept attack path finding](https://github.com/SpecterOps/bloodhound-docs/blob/main//analyze-data/bloodhound-gui/accept-findings).
</Note>
...
[bool](https://github.com/SpecterOps/bloodhound-docs/blob/main/Get-ADOptionalFeature -Identity 766ddcd8-acd0-445e-f3b9-a7f9b6744f2a | select -ExpandProperty EnabledScopes)
```
