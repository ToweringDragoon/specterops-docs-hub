...
Local Groups and Sessions can only be collected from domain-joined Windows systems, and require privileged collection to be configured, see [Why perform privileged collection in SharpHound](https://github.com/SpecterOps/bloodhound-docs/blob/main//collect-data/enterprise-collection/privileged-collection). This collection helps understand Attack Paths to individual systems based on non-centralized configurations.
...
<Note>[Least-Privileged Collection](https://github.com/SpecterOps/bloodhound-docs/blob/main//collect-data/enterprise-collection/least-privileged-collection) is possible for most BloodHound data types.</Note>

By default, computers beginning with Windows 10 version 1607 and Windows Server 2016 require Administrator access to perform RemoteSAM operations. This setting may be overridden with Group Policy for least-privilege collection. Microsoft documents supported Group Policy-based modifications for these permissions, alongside similar options for the registry scenarios discussed below.
...
<Note> Windows desktop operating systems do not have a local Print Operators group. Utilize membership in the local Administrators group or an alternate collection method as described in [Least-Privileged Collection](https://github.com/SpecterOps/bloodhound-docs/blob/main//collect-data/enterprise-collection/least-privileged-collection#sessions).</Note>
...
Two additional types of data can enhance the findings - [DC Registry](https://github.com/SpecterOps/bloodhound-docs/blob/main//collect-data/permissions#dc-registry) and [CA Registry](https://github.com/SpecterOps/bloodhound-docs/blob/main//collect-data/permissions#ca-registry).
...
Any Authenticated User may collect these registry key values by default.
