...
* SharpHound Enterprise **can** be hardened to mitigate the risk of Kerberos Delegation and NTLM authentication, see [SharpHound Enterprise Service Hardening](https://github.com/SpecterOps/bloodhound-docs/blob/main//manage-bloodhound/securing-bloodhound-and-collectors/sharphound-hardening).
...
* Creation of one BloodHound collector server for each tier, see [SharpHound Enterprise System Requirements](https://github.com/SpecterOps/bloodhound-docs/blob/main//install-data-collector/install-sharphound/system-requirements)
    * Tip: In BloodHound, mark the Tier Zero collector server as Tier Zero, see [Modifying Tier Zero](https://github.com/SpecterOps/bloodhound-docs/blob/main//get-started/security-boundaries/modifying-tier-zero)
* Creation of one SharpHound Enterprise service account for each tier, see [Create a gMSA for use with SharpHound Enterprise](https://github.com/SpecterOps/bloodhound-docs/blob/main//install-data-collector/install-sharphound/create-gmsa)
    * Each service account must have collection permission on all systems in the service account's tier - local `Administrators` group membership or [Least-Privileged Collection](https://github.com/SpecterOps/bloodhound-docs/blob/main//collect-data/enterprise-collection/least-privileged-collection) permissions.
    * Each service account is recommended to be hardened, see [SharpHound Enterprise Service Hardening](https://github.com/SpecterOps/bloodhound-docs/blob/main//manage-bloodhound/securing-bloodhound-and-collectors/sharphound-hardening).
    * Tip: In BloodHound, mark the Tier Zero service account as Tier Zero, see [Modifying Tier Zero](https://github.com/SpecterOps/bloodhound-docs/blob/main//get-started/security-boundaries/modifying-tier-zero)
...
1.  Follow the article [Create a SharpHound Enterprise collector client](https://github.com/SpecterOps/bloodhound-docs/blob/main//collect-data/enterprise-collection/create-collector)
...
2.  Install the collector client on the dedicated Tier Zero BloodHound collector server, using the dedicated Tier Zero SharpHound Enterprise service account. [See Install and Upgrade SharpHound Enterprise](https://github.com/SpecterOps/bloodhound-docs/blob/main//install-data-collector/install-sharphound/installation-upgrade).
...
1.  On the Tier Zero collector client, create a new collection schedule, see [Create a data collection schedule](https://github.com/SpecterOps/bloodhound-docs/blob/main//collect-data/enterprise-collection/collection-schedule)
