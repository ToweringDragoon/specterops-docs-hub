...
<Steps>
  ...
  <Step title="Enter query details">
    In the _Save Query_ dialog, enter a unique name and description for your query.
  ...
  <Step title="Share your query (optional)">
    In the _Manage Shared Queries_ dialog, select **Set to Public** to enable collaboration with all users in your BloodHound environment or select specific users. You can change these settings later if needed.
...
<CardGroup cols={1}>
  <Card title="Supported Cypher Syntax" icon="magnifying-glass" href="/analyze-data/cypher-supported" horizontal iconType="solid"/>
</CardGroup>
